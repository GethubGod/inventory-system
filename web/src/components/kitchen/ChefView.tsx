  ...
  if (row.kind === "pending") {
    return (
      <div className="flex items-center gap-2.5 py-3 border-b border-hairline last:border-b-0">
          ...
          {row.status === "failed" ? (
            <p className="text-xs font-semibold text-alert mt-0.5 flex items-center gap-1.5">
              ...
              <span className="truncate">
                Didn’t send — {row.error?.message ?? "kitchen never saw this"}
              </span>
            </p>
          ) : (
        ...
        {row.status === "failed" ? (
          <>
            {!row.error || isRetryableSendError(row.error.code) ? (
              <button
                type="button"
    ...
    <section>
      <div className="mb-3.5">
        ...
        <p className="text-[13px] text-ink2 mt-0.5">
          Tap an item, pick a quantity, confirm.
        </p>
      </div>
