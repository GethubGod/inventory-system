...
// the Deno entrypoint into the TypeScript program graph.
  ...
  return rows
    .map((row): CatalogItem | null => {
      ...
      return {
        ...
        default_order_unit: asNullableString(row.order_unit)
          ?? asNullableString(inventoryRow.default_order_unit),
      };
    })
...
export function buildCatalogFromInventoryItemRows(
  rows: Record<string, unknown>[],
): CatalogItem[] {
  return rows
    .map((row): CatalogItem | null => {
