-- Fix: include org_id in order_items INSERT.
-- The order_items table has a NOT NULL org_id column (FK to organizations)
-- that was missing from the previous RPC version, causing:
--   "null value in column org_id of relation order_items violates not-null constraint"
...
begin
  ...
  -- ── 4. Insert order items (only for new orders) ──
  ...
  -- ── 5. Fetch items with inventory details ──
  select jsonb_agg(
    jsonb_build_object(
      'id',                 oi.id,
